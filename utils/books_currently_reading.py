...
def new_book(title, author, genre, plot_summary, current_page, pages, user_id):
    sql = "INSERT INTO books_currently_reading (title,author, genre, plot_summary, current_page, pages, " \
          "user_id) VALUES (:title,:author, :genre, :plot_summary, :current_page, :pages, " \
          ":user_id) "
...
def show(user_id):
    sql = "SELECT book_id, title, author, genre, plot_summary,current_page, pages, user_id, CAST(round(cast(" \
          "current_page::float / pages::float as numeric)*100, 2) AS INTEGER)AS percentage FROM " \
          "books_currently_reading WHERE " \
